...
pub type SEL_CC_R = crate::FieldReader<u8, SEL_CC_A>;
...
impl From<SEL_CC_A> for u8 {
    #[inline(always)]
    fn from(variant: SEL_CC_A) -> Self {
        variant as _
    }
}
impl SEL_CC_R {
    #[doc = "Get enumerated values variant"]
    #[inline(always)]
    pub fn variant(&self) -> Option<SEL_CC_A> {
        match self.bits {
            0 => Some(SEL_CC_A::RISING_CAP0),
            1 => Some(SEL_CC_A::FALLING_CAP0),
            4 => Some(SEL_CC_A::RISING_CAP1),
            5 => Some(SEL_CC_A::FALLING_CAP1),
            _ => None,
        }
    }
...
impl R {
    ...
    #[inline(always)]
    pub fn sel_cc(&self) -> SEL_CC_R {
        SEL_CC_R::new(((self.bits >> 5) & 7) as u8)
    }
}
impl W {
    #[doc = "Bits 0:1 - Counter/Timer Mode. This field selects which rising PCLK edges can increment Timer's Prescale Counter (PC), or clear PC and increment Timer Counter (TC). If Counter mode is selected in the CTCR, bits 2:0 in the Capture Control Register (CCR) must be programmed as 000."]
    ...
    #[inline(always)]
    #[must_use]
    pub fn sel_cc(&mut self) -> SEL_CC_W<5> {
        SEL_CC_W::new(self)
    }
